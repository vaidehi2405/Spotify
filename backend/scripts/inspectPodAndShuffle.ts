...
async function main() {
  ...
  const shuffleReviews = rawReviews.filter(r => r.review_text.toLowerCase().includes('shuffle'));

  console.log(`--- PODCAST REVIEWS (${podcastReviews.length}) ---`);
  podcastReviews.forEach((r, idx) => {
    console.log(`[${idx+1}] ID: ${r.id} | "${r.review_text}"`);
  });

  console.log(`\n--- SHUFFLE REVIEWS (${shuffleReviews.length}) ---`);
  shuffleReviews.forEach((r, idx) => {
    console.log(`[${idx+1}] ID: ${r.id} | "${r.review_text}"`);
  });
}
